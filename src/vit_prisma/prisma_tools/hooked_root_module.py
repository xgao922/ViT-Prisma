...
class HookedRootModule(nn.Module):
    ...
    def remove_all_hook_fns(self, dir="both", including_permanent=False, level=None):
    ...
    def run_with_cache(
        ...
        **model_kwargs,
        ...
        # Get caching hooks
        cache_dict, cache_fwd, cache_bwd = self.get_caching_hooks(
            names_filter, incl_bwd, device, remove_batch_dim=remove_batch_dim
        )

        # Combine caching hooks with provided hooks
        combined_fwd_hooks = fwd_hooks + cache_fwd
        combined_bwd_hooks = bwd_hooks + cache_bwd

        # Run with combined hooks
        with self.hooks(
            fwd_hooks=combined_fwd_hooks,
            bwd_hooks=combined_bwd_hooks,
            reset_hooks_end=reset_hooks_end,
            clear_contexts=clear_contexts,
        ):
            model_out = self(*model_args, **model_kwargs)
            if incl_bwd or bwd_hooks:
                model_out.backward()
